 ...
 */
int
dialog_oskbox(const char *title, const char *cprompt, int height, int width,
		const char *init, const int password)
{
  ...
  retry:
    ...
    dlg_register_window(dialog, "oskbox", binding);
    dlg_register_buttons(dialog, "oskbox", buttons);
    ...
    dlg_register_window(editor, "oskbox2", binding2);
